...
old = '''          if (state.mode !is ScreenMode.ArticleEditor && state.mode !is ScreenMode.Stocks) {'''
new = '''          if (state.mode !is ScreenMode.ArticleEditor && state.mode !is ScreenMode.Stocks && state.mode !is ScreenMode.AddDailySummary) {'''

if old not in s:
    raise SystemExit("top toolbar visibility condition not found")

s = s.replace(old, new, 1)
screen.write_text(s)

# ---------- AddDailySummaryScreen.kt ----------
daily.write_text('''package com.ml.app.ui
...
git add app/src/main/java/com/ml/app/ui/SummaryScreen.kt app/src/main/java/com/ml/app/ui/AddDailySummaryScreen.kt
git commit -m "move daily summary expenses into each bag block and hide top date bar"
git push
