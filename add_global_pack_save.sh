...
root = Path("app/src/main/java/com/ml/app")
data_dir = root / "data"
ui_file = root / "ui" / "AddEditArticleScreen.kt"

# ---------- 1. New helper: PackUploadManager.kt ----------
pack_upload = data_dir / "PackUploadManager.kt"
pack_upload.write_text('''package com.ml.app.data
...
# ---------- 2. Patch AddEditArticleScreen.kt ----------
u = ui_file.read_text()
...
if "val scope = rememberCoroutineScope()" not in u:
    u = u.replace(
...
\s*\) \{
\s*Text\(if \(selectedBagId\.isNullOrBlank\(\)\) "Сохранить" else "Сохранить изменения"\)
\s*\}''',
    re.DOTALL
)
...
u2, count = save_pattern.subn(new_save, u, count=1)
if count == 0:
    raise SystemExit("save button block not found")
ui_file.write_text(u2)

print("patched")
PY

git add app/src/main/java/com/ml/app/data/PackUploadManager.kt \
        app/src/main/java/com/ml/app/ui/AddEditArticleScreen.kt
git commit -m "add global pack save and upload"
git push
