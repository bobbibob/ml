...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (path === "/debug_create_reminder_task" && request.method === "POST") {
        ...
        const title = String(body?.title || "debug reminder task").trim() || "debug reminder task"
        const assigneeUserId = String(body?.assignee_user_id || "").trim()
        const createdByUserId = String(body?.created_by_user_id || "debug_admin").trim() || "debug_admin"
        const reminderType = String(body?.reminder_type || "interval").trim() || "interval"
        const reminderIntervalMinutes = Number(body?.reminder_interval_minutes ?? 10)
        const reminderTimeOfDay = body?.reminder_time_of_day == null ? null : String(body.reminder_time_of_day).trim() || null

        if (!assigneeUserId) {
          return json({ ok: false, error: "assignee_user_id required" }, 400)
        }

        const taskId = "dbg_" + Math.random().toString(16).slice(2, 14)
        ...
        await env.DB.prepare(`
          INSERT INTO tasks (
            ...
            last_reminder_sent_at, next_reminder_at,
            is_urgent, client_request_id, created_at, updated_at
          ) VALUES (?, ?, ?, 'open', ?, ?, ?, ?, ?, ?, ?, 0, NULL, ?, ?)
        `).bind(
          taskId,
          title,
          "created by ml-reminders-debug",
          createdByUserId,
          assigneeUserId,
          reminderType,
          Number.isFinite(reminderIntervalMinutes) ? reminderIntervalMinutes : null,
      ...
      if (path === "/debug_find_task" && request.method === "GET") {
        ...
        const rows = await env.DB.prepare(`
          SELECT
            task_id, title, status, assignee_user_id, created_by_user_id,
            reminder_type, reminder_interval_minutes, reminder_time_of_day,
            last_reminder_sent_at, next_reminder_at, created_at, updated_at
          FROM tasks
          WHERE title = ?
      ...
      if (path === "/debug_run_reminders" && request.method === "POST") {
        ...
        return json({ ok: true, processed, sent: sentTotal, now })
      }
