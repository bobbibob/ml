...
class SummaryViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun openArticleEditor(bagId: String?) { _state.update { it.copy(mode = ScreenMode.ArticleEditor(bagId)) } }
    fun backToTimeline() { refreshTimeline() }
    fun backFromArticlePicker() = backToTimeline()
    fun backFromArticleEditor() = openArticlePicker()
    fun syncIfChanged() { if (state.value.mode is ScreenMode.Details) openDetails(state.value.selectedDate) else refreshTimeline() }
}
