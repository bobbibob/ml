                            ...
                            Thread {
                                ...
                                if (needsRetry) {
                                    ...
                                    if (parserRetryCount >= 3) {
                                        statusText = "Комплектные заказы раскрыты, повторяем парсинг без повторного раскрытия..."
                                        webView.postDelayed({
                                            webView.evaluateJavascript("window.__ml_skip_expand = true;") { }
                                            parserRetryCount = 0
                                            runParser()
                                        }, 2200)
                                    } else {
                                ...
                                webView.evaluateJavascript("window.__ml_skip_expand = false;") { }
