...
class SQLiteRepo(private val context: Context) {
  ...
  private fun normalizeColor(raw: String?, bag: String?): String? {
    ...
    bag?.let {
      if (low == it.trim().lowercase()) return null
    }
    ...
    openDb().use { db ->
      ...
      for (bag in bags) {
        ...
        db.rawQuery(
          ...
          SELECT *
          FROM svodka
          WHERE date=? AND bag=? AND color IN ("__TOTAL__","TOTAL")
          LIMIT 1
          """.trimIndent(),
          arrayOf(date, bag)
        ).use { c ->
          if (c.moveToFirst()) {
            ...
            if (io >= 0 && !c.isNull(io)) totalOrders = c.getDouble(io)
        ...
        db.rawQuery(
          ...
          while (c.moveToNext()) {
            ...
            val o = c.getDouble(io)
            val s = c.getDouble(isx)
